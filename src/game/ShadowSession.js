...
export class ShadowSession {
  ...
  reset() {
    ...
    this.state = {
      ...
      player: null,
      combo: EMPTY_COMBO(),
      stats: { jab: 0, cross: 0, hook: 0, combos: 0, dodgeLeft: 0, dodgeRight: 0, guardSeconds: 0 },
    };
    this._syncState();
    return this.state;
  }
  ...
  act(input) {
    ...
    this._expireCombo();
    if (this._guardHeld || input.startsWith('dodge')) this._clearCombo();
  ...
  update(realSeconds, animationSeconds = realSeconds) {
    ...
    if (action) {
      ...
      if (!action.counted && action.elapsed + EPSILON >= action.peak) {
        ...
        const event = { type: 'motion', action: action.action, time: action.startedAt + action.peak };
    ...
    if (this._guardHeld) this.state.stats.guardSeconds += Math.max(0, dt - busyFor) / dt * realSeconds;
    this._syncState();
  }
  ...
  _syncState() {
    ...
    this.state.combo = {
      ...
      ready: this.state.phase === 'running' && !action && !this._guardHeld && this._combo?.step === 2,
