...
export const OPPONENT_PROFILES = Object.freeze({
  ...
  pablo: Object.freeze({
    id: 'pablo', name: 'Pablo', shortName: 'Pablo', official: false, region: 'mexico', coach: 'Octopus',
    eyebrow: 'MEXIQUE · PARTENAIRE DE SPARRING',
    introduction: 'Pablo travaille le jab, puis le direct au corps. Observe sa préparation et réponds pendant son retour en garde.',
    rhythm: Object.freeze({ initialOpening: 1.2, afterRecovery: 1.8 }),
    pattern: Object.freeze([
      { action: 'guard', duration: .9, guardLevel: 'head' },
      { attack: 'jab', target: 'head', tell: .85, opening: .65 },
      { attack: 'cross', target: 'body', tell: .95, opening: 1.8 },
    ].map(Object.freeze)),
  }),
  danielo: Object.freeze({
    id: 'danielo', name: 'Danielo', shortName: 'Danielo', official: true, region: 'mexico', coach: 'Octopus',
    maxResistance: 132, attackPower: 1.85, eyebrow: 'MEXIQUE · L’ARÈNE DE TERRE',
    introduction: 'Danielo plante ses appuis avant ses directs. Sa garde descend après le premier échange : attends le retour de son épaule, puis réponds sans t’épuiser.',
    advice: 'Son direct au corps précède souvent le jab haut. Garde du souffle pour la deuxième frappe; sa longue récupération vient après le direct à la tête.',
    rhythm: Object.freeze({ initialOpening: .75, afterRecovery: 1.4 }),
    pattern: Object.freeze([
      { action: 'guard', duration: .75, guardLevel: 'head' },
      { attack: 'cross', target: 'body', tell: .78, opening: .35 },
      { attack: 'jab', target: 'head', tell: .60, opening: .45 },
      { action: 'guard', duration: .40, guardLevel: 'body' },
      { attack: 'cross', target: 'head', tell: .68, opening: 1.5 },
      { action: 'feint', duration: .35, side: 'right' },
      { attack: 'jab', target: 'head', tell: .62, opening: 1.35 },
    ].map(Object.freeze)),
  }),
  'gold-rios': Object.freeze({
    id: 'gold-rios', name: 'Rafael Ríos', shortName: 'Ríos', official: true, tournament: true, tournamentTier: 'gold',
    maxResistance: 140, attackPower: 1.9, eyebrow: 'GANTS DORÉS · QUART DE FINALE',
    introduction: 'Ríos travaille derrière deux jabs précis, puis change de hauteur. Sa série est rapide, mais chaque épaule annonce clairement le coup suivant.',
    advice: 'Ne réponds pas entre ses deux jabs. Protège ensuite le corps, puis profite du grand retour en garde.',
    rhythm: Object.freeze({ initialOpening: .7, afterRecovery: 1.35 }),
    pattern: Object.freeze([
      { action: 'guard', duration: .55, guardLevel: 'head' },
      { attack: 'jab', target: 'head', tell: .58, opening: .25 },
      { attack: 'jab', target: 'head', tell: .56, opening: .25 },
      { attack: 'cross', target: 'body', tell: .70, opening: 1.4 },
      { action: 'guard', duration: .55, guardLevel: 'body' },
      { attack: 'cross', target: 'head', tell: .65, opening: 1.3 },
    ].map(Object.freeze)),
  }),
  'gold-moreau': Object.freeze({
    id: 'gold-moreau', name: 'Émile Moreau', shortName: 'Moreau', official: true, tournament: true, tournamentTier: 'gold',
    maxResistance: 146, attackPower: 1.95, eyebrow: 'GANTS DORÉS · DEMI-FINALE',
    introduction: 'Moreau est solide et change souvent de garde. Il feinte avant son direct lourd : reste calme jusqu’à la vraie préparation.',
    advice: 'Sa feinte ne touche pas. Observe ses genoux pour le corps et son épaule pour la tête. Attaque la hauteur libre pendant sa garde.',
    rhythm: Object.freeze({ initialOpening: .7, afterRecovery: 1.3 }),
    pattern: Object.freeze([
      { action: 'guard', duration: .7, guardLevel: 'head' },
      { action: 'feint', duration: .32, side: 'right' },
      { attack: 'cross', target: 'body', tell: .72, opening: .3 },
      { attack: 'cross', target: 'head', tell: .64, opening: 1.4 },
      { action: 'guard', duration: .6, guardLevel: 'body' },
      { attack: 'jab', target: 'head', tell: .56, opening: .28 },
      { attack: 'cross', target: 'body', tell: .68, opening: 1.35 },
    ].map(Object.freeze)),
  }),
  'gold-santos': Object.freeze({
    id: 'gold-santos', name: 'Thiago Santos', shortName: 'Santos', official: true, tournament: true, tournamentTier: 'gold',
    maxResistance: 150, attackPower: 2, eyebrow: 'GANTS DORÉS · FINALE',
    introduction: 'Santos réunit les leçons du voyage : appuis, feinte et changements de hauteur. Ses trois coups sont lisibles; garde ta réponse pour la fin de la série.',
    advice: 'Il ouvre au corps, remonte au jab, puis termine par un direct haut. Ne poursuis pas son esquive. Trois protections propres, puis ton enchaînement.',
    rhythm: Object.freeze({ initialOpening: .65, afterRecovery: 1.3 }),
    pattern: Object.freeze([
      { action: 'dodge', duration: .55, side: 'left' },
      { action: 'guard', duration: .35, guardLevel: 'head' },
      { attack: 'cross', target: 'body', tell: .65, opening: .25 },
      { attack: 'jab', target: 'head', tell: .55, opening: .25 },
      { attack: 'cross', target: 'head', tell: .60, opening: 1.35 },
      { action: 'guard', duration: .55, guardLevel: 'body' },
      { action: 'feint', duration: .3, side: 'left' },
      { attack: 'jab', target: 'head', tell: .56, opening: .28 },
      { attack: 'cross', target: 'body', tell: .64, opening: 1.4 },
    ].map(Object.freeze)),
  }),
  runner: Object.freeze({
    id: 'runner', name: 'Le coureur pressé', shortName: 'Le coureur', official: false, streetFight: true,
    maxResistance: 90, attackPower: .75, eyebrow: 'SUR LE PARCOURS',
    introduction: 'Il s’énerve et lance de grands coups maladroits. Protège-toi et profite de ses ouvertures. Une seule chute termine l’altercation; la course continue.',
    rhythm: Object.freeze({ initialOpening: 1.1, afterRecovery: 2 }),
    pattern: Object.freeze([
      { action: 'guard', duration: .85, guardLevel: 'head' },
      { attack: 'cross', target: 'head', tell: 1.0, opening: 1.8 },
      { attack: 'jab', target: 'head', tell: .8, opening: 1.45 },
      { attack: 'cross', target: 'body', tell: 1.05, opening: 2 },
    ].map(Object.freeze)),
  }),
