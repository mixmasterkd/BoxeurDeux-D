...
export function setSceneShell(mode) {
  ...
  document.getElementById('stage').setAttribute('aria-label', { gym: 'Visite du gym', sparring: 'Sparring avec Rémi le Tank', bag: 'Entraînement au sac de frappe' }[mode]);
  document.getElementById('game').setAttribute('aria-label', mode === 'gym'
    ...
    : mode === 'bag' ? 'Votre boxeur à tuque rouge pratique ses enchaînements au sac.' : 'Ring de boxe en pixel art. Vous êtes de dos face à Rémi le Tank.');
  for (const scene of ['gym', 'sparring', 'bag']) document.getElementById(`${scene}-ui`).hidden = mode !== scene;
  document.getElementById('game-commands').innerHTML = GUIDES[mode];
  document.querySelector('.prototype-label').textContent = { gym: 'LE GYM', sparring: 'SPARRING AVEC RÉMI', bag: 'SAC · ENCHAÎNEMENTS' }[mode];
}
