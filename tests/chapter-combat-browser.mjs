...
function fixture(opponent) {
  const memory = new Map(), profile = new CareerProfile({storage:{getItem:k=>memory.get(k)??null,setItem:(k,v)=>memory.set(k,v),removeItem:k=>memory.delete(k)}});
  profile.recordFight({opponent:'beton',winner:'player',score:10});
  if (opponent !== 'kramer') {
    profile.recordFight({opponent:'kramer',winner:'player',score:10});
    for (let tour=0;tour<6;tour++) {
      if (!profile.canStartActivity('delivery').ok) profile.sleep();
      assert.ok(profile.startDelivery().ok);
      for (const stop of DELIVERY_STOPS) assert.ok(profile.deliverParcel(stop,{tip:2}).ok);
    }
    assert.ok(profile.startTournament().ok);
    for(const previous of TOURNAMENT_OPPONENTS.slice(0,TOURNAMENT_OPPONENTS.indexOf(opponent))) {
      assert.ok(profile.recordTournamentFight({opponent:previous,winner:'player',score:10,matchId:profile.tournamentStatus().currentMatchId}).ok);
      assert.ok(profile.sleep().ok);
    }
    profile.setLocation({scene:'hotel-venue',x:640,y:560,facing:'up'});
  }
...
const selected=process.env.CHAPTER_COMBAT_CASE?.split(',');
const small=process.env.CHAPTER_COMBAT_SMALL==='1', suffix=small?'-small-mobile':'';
const reportPath=process.env.CHAPTER_COMBAT_REPORT??'docs/chapter-combat-browser-results.json';
const cases=[['kramer',1440,1000,false],['bellini',1440,1000,false],['fortin',844,390,true],['gagnon',568,320,true]].filter(([id])=>!selected||selected.includes(id)).map(row=>small?[row[0],568,320,true]:row);
...
async function confirm(page,cdp){await page.locator('#sparring-ui .primary-button').focus();if(cdp)await tapContact(page,cdp,'#sparring-ui [data-pad-button="a"]');else await page.keyboard.press('Enter');}
...
try {
  for(const [id,width,height,mobile] of cases){
    ...
    assert.equal((await state(page)).settings.opponent,id);
    await confirm(page,cdp);await wait(page,()=>window.__sparring.session.state.phase==='running');
    const input=await controller(page,cdp);let queue=[],used=false,shot=false,surrender=false,paused=false;const measuredCounts=new Set();const deadline=Date.now()+240000;
    while(Date.now()<deadline){
      const s=await state(page);if(s.phase==='finished')break;
      if(s.phase==='between'){await input.guard(null);queue=[];await page.screenshot({path:`docs/chapter-${id}-corner.png`});await confirm(page,cdp);await page.waitForTimeout(150);continue;}
      ...
      if(s.phase==='knockdown'){
        ...
        if(s.bout.count.stage==='surrender'&&s.bout.count.elapsed>=.9&&!surrender){await page.screenshot({path:`docs/chapter-kramer-surrender${suffix}.png`});surrender=true;}
      ...
      if(!shot&&r.action.startsWith('tell')){await page.screenshot({path:`docs/chapter-${id}-${mobile?'mobile':'desktop'}.png`});await geometry(page,mobile,id);shot=true;}
      if(!paused&&s.elapsed>2){
        await input.guard(null);if(cdp)await tapContact(page,cdp,'#sparring-ui .console-menu-button');else await page.keyboard.press('p');
        await wait(page,()=>window.__sparring.session.state.phase==='paused');const frozen=await state(page);await page.waitForTimeout(200);assert.deepEqual(await state(page),frozen);await confirm(page,cdp);paused=true;continue;
      }
      ...
      if(!queue.length&&!used&&opening&&r.duration*(1-r.progress)>=1.6&&s.player.action==='idle'&&s.stamina>=48){queue=['jab','cross','jab'];used=true;}
    ...
    if(id==='kramer'){assert.equal(final.bout.result.reason,'abandon');assert.equal(final.bout.downs.remi.total,2);assert.equal(surrender,true);}
    const impacts=await page.evaluate(()=>window.__sparring.impacts);assert.ok(impacts.some(i=>i.type==='player-hit'));assert.ok(impacts.some(i=>i.type==='remi-blocked'));assert.ok(impacts.filter(i=>i.type==='player-hit').every(i=>i.playerTexture.startsWith(id==='kramer'?'player-':'competition-')));
    const stored=await page.evaluate(key=>JSON.parse(localStorage.getItem(key)),CAREER_STORAGE_KEY);
    assert.equal(stored.fights[id].wins,1);if(id!=='kramer')assert.equal(stored.tournament.active.results.at(-1).opponent,id);
    if(mobile){
    ...
    await page.screenshot({path:`docs/chapter-${id}-result${suffix}.png`});
    ...
    if(id!=='kramer'){
      await confirm(page,cdp);await wait(page,()=>!window.__sparring&&document.querySelector('#gym-ui')?.hidden===false,null,60000);
      await page.screenshot({path:`docs/chapter-${id}-return-hotel.png`});
      const after=await page.evaluate(key=>JSON.parse(localStorage.getItem(key)),CAREER_STORAGE_KEY);assert.equal(after.tournament.active.results.at(-1).opponent,id);assert.equal(after.fights[id].wins,1);
      reports.push({id,returnToHotel:true,location:after.location});
    }
    await context.close();
  }
  assert.deepEqual(errors,[]);
}catch(error){failure=error.stack;console.error(error);if(active&&!active.isClosed())await active.screenshot({path:'docs/chapter-combat-failure.png'}).catch(()=>{});process.exitCode=1;}
