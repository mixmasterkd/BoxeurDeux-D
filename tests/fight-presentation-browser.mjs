...
try{
 for(const mobile of [false,true]) for(const tournament of [false,true]){
 const context=await browser.newContext({viewport:mobile?{width:568,height:320}:{width:1280,height:900},isMobile:mobile,hasTouch:mobile});await suppressHotReload(context);const page=await context.newPage();const errors=[];page.on('pageerror',e=>errors.push(e.message));
 await page.goto(`http://127.0.0.1:5173/?scene=fight&opponent=${tournament?'gold-santos':'beton'}`);await wait(page,()=>window.__sparring?.scene.decisionView);
 for(const winner of ['player','remi']){
 const decision=judgeBout([1,2,3].map(round=>({round,stats:{landed:winner==='player'?3:1,received:winner==='remi'?3:1},downs:{player:0,remi:0}})),{judges:tournament?5:3});
 ...
 const readable=await page.locator('.judge-cards').evaluate(e=>[...e.querySelectorAll('b')].every(b=>{const r=b.getBoundingClientRect(),hit=document.elementFromPoint(r.x+r.width/2,r.y+r.height/2);return hit===b||b.contains(hit);}));assert.ok(readable,'all scores visible without scrolling');
 const info=await page.evaluate(()=>{const panel=document.querySelector('.round-panel').getBoundingClientRect(),c=document.querySelector('canvas').getBoundingClientRect();return {top:(panel.top-c.top)/c.height*720,height:panel.height/c.height*720,hasCards:!!document.querySelector('.judge-cards').getClientRects().length,referee:window.__sparring.scene.decisionView.referee.texture.key,player:window.__sparring.scene.decisionView.images.player.texture.key,cards:document.querySelector('.judge-cards').children.length};});
 ...
 assert.equal(info.cards,tournament?5:3);assert.equal(info.player,`${tournament?'decision-player':'decision-local'}-${winner==='player'?'winner':'loser'}`);
 await page.screenshot({path:`docs/decision-fixture-${mobile?'mobile':'desktop'}-${tournament?'gold-':''}${winner}.png`});
 ...
 assert.deepEqual(errors,[]);await context.close();console.log(`${mobile?'Mobile568':'Desktop'}: ${tournament?'5':'3'}cards, bothverdicts, E card details and breathing target fit.`);
 }
