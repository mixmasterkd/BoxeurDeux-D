 ...
 while(clock<360&&game.state.phase!=='finished'){
  ...
  if(s.phase==='running'){
   ...
   if(defend&&s.player.action==='idle'){
    ...
    else if(s.remi.action==='open'&&s.remi.duration>=1.3&&s.remi.progress<.06&&s.stamina>=48){if(game.act('jab'))sequence='cross';}
   }
  }else sequence=null;
...
for(const id of ids){
 ...
 test(`${id}: full preparations and head/body choices are fixed independently of input`,()=>{
  ...
  for(let frame=0;frame<700;frame++){
   ...
   // A real knockdown freezes that bout's clock; it is not reactive AI.
   if(a.state.phase!=='running'||b.state.phase!=='running')break;
   comparedFrames++;
  ...
  assert.ok(comparedFrames>=360, `${id}: compare at least six active seconds`);
 });
}
test('Le Feu rises from body to head in two then three punches, followed by time for a complete counter',()=>{
 ...
 for(const stage of pattern){if(stage.attack){current.push(stage.target);if(stage.opening>=1.3){bursts.push(current);current=[];}}}
 assert.deepEqual(bursts,[['body','head'],['body','head','head']]);
 assert.ok(pattern.filter(s=>s.attack&&s.opening>=1.3).every(s=>s.opening>=1.3));
});
