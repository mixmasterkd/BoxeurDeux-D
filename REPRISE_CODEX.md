...
> **Publication demandée le 10 septembre 2026 :** la capture de `mixmasterkd.github.io/BoxeurDeux-D/` confirme que l’utilisateur souhaite y jouer au sparring actuel. L’interdiction initiale de publication est dépassée pour ce site. Le prototype complet est enregistré dans Git; `.github/workflows/pages.yml` prépare la compilation et le déploiement de `dist/`. Vérifier l’envoi des commits et la réussite du déploiement avant d’annoncer le jeu en ligne. Le terminal de cette session ne dispose pas de l’authentification GitHub de VS Code : ne jamais demander de mot de passe ou jeton dans le chat; utiliser l’envoi depuis VS Code au besoin. Ne pas confondre un test local de `dist/` et un test de l’adresse publique.
