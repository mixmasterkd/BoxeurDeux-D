...
L’état exact des vérifications et de la publication est consigné dans [docs/VERIFICATIONS.md](docs/VERIFICATIONS.md).
...
2. Sortez dans le quartier. Le **gym** donne accès au sac, au miroir, à la corde, à la speed ball et à Rémi. La **salle communautaire** ouvre les combats sans intérieur supplémentaire.
3. Le passage ouvert **à gauche du quartier** mène à la **rue des livreurs**. Le dépôt est au sud; les trois portes numérotées 12, 24 et 36 sont au nord.
...
| --- | --- | --- |
| Se déplacer / choisir dans un menu | Flèches, WASD ou ZQSD | Joypad gauche |
| Interagir / confirmer | E ou Entrée | A |
| Revenir dans un menu | Échap | B |
| Pause et aide **Commandes** | P ou Échap | ☰ |
| Jab gauche / direct droit | J / K | A / B |
| Garde haute / basse | Haut / bas | Joypad haut / bas |
| Esquive gauche / droite | Gauche / droite | Joypad gauche / droite |
| Coup au corps | Bas + J ou K | Joypad bas + A ou B |
| Couper / rétablir le son | M, ou menu | Menu pause |

**J → K → J** (ou **A → B → A**) permet jab, direct, crochet gauche : après le retour en garde, enchaînez dans la fenêtre de 0,5 seconde. Les frappes ne sont pas mises en attente; maintenir un bouton ne répète pas le coup. La garde et les diagonales utiles acceptent les appuis simultanés.
...
Au dépôt, prendre une tournée coûte **30 énergie quotidienne** une seule fois. Le vélo est prêté. Livrez dans l’ordre les **trois adresses annoncées**; une autre porte ne paie pas le colis. La tournée et les livraisons réglées sont sauvegardées.
...
| Rue Nord | Survêtement bleu nuit / bordeaux | 28 $ / 36 $ |
...
| Pads à l’hôtel · 45 s | 10 | 12 mouvements, 60 % | Puissance +1 |
| Piscine de l’hôtel · 45 s | 10 | 3 longueurs, 60 % | Endurance maximale +2 |

Le sac annonce ses enchaînements; la corde et la speed ball utilisent J/K ou A/B en alternance. Rémi enseigne les mêmes gardes, esquives et frappes que les combats. Les touches sont comptées au contact des animations, avec une distinction entre coup net, blocage et esquive. Chaque accueil annonce son coût, son objectif et son plafond; le bilan indique le gain obtenu ou la raison de son absence.
...
Le sparring libre avec **Rémi le Tank** reste un round de 60 secondes au gym. **Résistance et relevés** et les combats officiels utilisent au plus **trois rounds de 60 secondes**, avec Rémi coach au coin entre les reprises.
...
- **Au coin** : conseil de Rémi, endurance pleine à son maximum personnel, résistance +20 sans dépasser son maximum; seul le compteur de chutes du round est remis à zéro. La reprise attend le joueur.
...
Le séjour comporte plusieurs lieux reliés par un ascenseur : **chambre 201**, **couloir avec d’autres chambres**, **réception**, **mini-gym**, **piscine** et **grande salle d’événement**. La caméra suit le personnage dans le couloir et la salle. Quatre rings, des gradins et des participants donnent une ambiance de rencontre; le joueur dispute ses combats au ring 1. Le tableau près de l’entrée affiche huit participants et les résultats.
...
Aux **pads**, Rémi présente les cibles et demande aussi gardes ou esquives : mêmes mouvements qu’en combat. À la **piscine**, alternez les bras au repère; douze bonnes poussées font une longueur. Ces activités coûtent de l’énergie et donnent des capacités plafonnées; la piscine ne recharge pas la journée.
...
npm run test:chapter-world    # Vrais trajets clavier/tactile, livraisons et boutiques
npm run test:chapter-combat   # Kramer et adversaires du tournoi dans le navigateur
npm run test:hotel            # Hôtel, ascenseur, pads, piscine et commandes
npm run test:exploration      # Régression maison/quartier/gym, sommeil et reprise
npm run test:static           # Bundle compilé local sans serveur concurrent
```
...
Les accès `?scene=home`, `neighborhood`, `residential`, `commercial`, `clothing-shop`, `boxing-shop`, `gym`, `bag`, `shadow`, `rope`, `speedball` et `fight&opponent=kramer` sont des raccourcis de développement. Ils ne donnent ni argent ni énergie ni qualification. Les lieux `hotel-*`, `pads` et `pool` nécessitent une inscription active.
...
La suite reste à discuter : plus de collections, réglages de difficulté après essais, adversaires et quartiers supplémentaires. Les futurs ordinateurs, cellulaires et achats en ligne ne font pas partie de ce chapitre. La liste initiale et son historique restent dans [docs/PROCHAINES_ETAPES.md](docs/PROCHAINES_ETAPES.md).
