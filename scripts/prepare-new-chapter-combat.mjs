...
const ref='references/characters/new-chapter-combat';
...
export function clean(name,columns,rows){
...
if(import.meta.url === pathToFileURL(process.argv[1]).href) for(const id of process.argv.slice(2).length ? process.argv.slice(2) : ['pablo','danielo','gold-rios','gold-moreau','gold-santos','runner','runner-player']){
 ...
 pieces.forEach((box,index)=>{
  const name=id === 'runner-player' && index === 15 ? 'hook' : names[index],p=id === 'runner-player' ? [[.60,.14,.52,.34],[.60,.14,.52,.34],[.47,.14,.5,.34,.9,.19],[.59,.14,.58,.35,.07,.19],[.63,.13,.5,.36,.10,.25],[.40,.14,.48,.36,.92,.18],[.57,.15,.5,.38,.66,.28],[.45,.14,.3,.4,.95,.26],[.57,.14,.5,.35],[.32,.16,.5,.4],[.65,.14,.5,.35],[.69,.15,.55,.34],[.5,.19,.5,.44],[.65,.15,.5,.35],[.32,.12,.45,.34],[.55,.15,.5,.35,.12,.22]][index] : points[index],pixels=Buffer.alloc(image.pixels.length);
  ...
  atlas.poses[`${id}-${name}`]={...extract({...image,pixels},box,`opponents/${id}/${id}-${name}.png`,{width:640,canvasHeight:640,baseline:624,center:320,scale,sourceCenter,landmarks:{head:relative(p[0],p[1]),body:relative(p[2],p[3]),...(p.length>4?{glove:relative(p[4],p[5])}:{})}}),sourceFile:`${ref}/${id}-alpha.png`,physicalScale:scale,...(id === 'runner-player' && ['jab-windup','cross-windup'].includes(name) ? {mirror:true} : {})};
 });
 if(id === 'runner-player') {
  for(const [name,source,mirror] of [['hook-windup','jab-windup',false],['hook-recover','jab-windup',false],['jab-body','cross-body',true],['jab-windup-body','cross-windup-body',true],['hook-body','cross-body',true],['hook-windup-body','cross-windup-body',true]]) {
    fs.copyFileSync(path.join(dir,`${id}-${source}.png`),path.join(dir,`${id}-${name}.png`));
    atlas.poses[`${id}-${name}`]={...atlas.poses[`${id}-${source}`],mirror};
  }
}
