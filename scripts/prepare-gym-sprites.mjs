...
// Usage: node scripts/prepare-gym-sprites.mjs --source references/characters/exploration/player-alpha.png
import fs from 'node:fs';
import path from 'node:path';
import { readPng, writePng, extract } from './sprite-png.mjs';

const root = path.resolve(import.meta.dirname, '..');
const output = path.join(root, 'public/assets/sprites/exploration');
...
if (sourceArgument < 0 || !args[sourceArgument + 1]) {
  throw new Error('Supply an RGBA source with --source references/characters/exploration/<filename>.png');
}
...
// Rémi has his own original RPG pose, sharing the same canvas and foot anchor.
const remiFile = path.join(root, 'references/characters/exploration/remi-alpha.png');
if (fs.existsSync(remiFile)) {
  const remiImage = readPng(remiFile);
  const remi = extract(remiImage, { x: 0, y: 0, w: remiImage.width, h: remiImage.height }, 'exploration/remi.png', {
    height: 94, width, canvasHeight: height, baseline: anchor.y, center: anchor.x,
  });
  fs.writeFileSync(path.join(output, 'remi.json'), `${JSON.stringify(remi, null, 2)}\n`);
}
