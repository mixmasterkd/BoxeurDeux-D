...
// Usage: node scripts/prepare-street-sprites.mjs --source references/characters/street/player-alpha.png
import fs from 'node:fs';
import path from 'node:path';
import { readPng, writePng } from './sprite-png.mjs';

const root = path.resolve(import.meta.dirname, '..');
const output = path.join(root, 'public/assets/sprites/street');
...
if (sourceArgument < 0 || !args[sourceArgument + 1]) {
  throw new Error('Supply an RGBA source with --source references/characters/street/<filename>.png');
}
